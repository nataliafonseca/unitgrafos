...
from lib.grafo import definir_grafo, Grafo
...
while True:
    resposta = menu(["Definir Grafo", "Imprimir Matriz de Adjacencia",
                     "Imprimir Estrutura de Adjacencia", "Sair do sistema"])

    if resposta == 1:
        print()
        cabecalho("Opção 1 - Entrada de Grafo")
        grafo = definir_grafo()
        print()

    elif resposta == 2:
        print()
        cabecalho("Opção 2 - Imprimir Matriz de Adjacencia")
        if grafo == grafo_exemplo:
        ...
        print()
        cabecalho("Opção 3 - Imprimir Estrutura de Adjacencia")
        if grafo == grafo_exemplo:
            print(Fore.RED + "ATENÇÃO! Você ainda não definiu um grafo, "
